...
export async function getSqlServerPool(): Promise<ConnectionPool> {
    ...
    const pool = (global as any)._sqlConnectionPool as ConnectionPool;
    
    // Asignamos el pool no nulo a la variable de módulo para el caché
    connectionPool = pool; 
    
    // Retornamos la variable local que sabemos que es ConnectionPool
    return pool;
}
